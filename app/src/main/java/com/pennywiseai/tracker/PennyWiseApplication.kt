...
class PennyWiseApplication : Application(), Configuration.Provider {
    ...
    @Inject
    lateinit var slipMediaObserver: com.pennywiseai.tracker.slip.observer.SlipMediaObserver
    ...
    override fun onCreate() {
        ...
        // Register background Slip Observer to automatically detect newly saved bank slips
        try {
            slipMediaObserver.register()
        } catch (e: Exception) {
            android.util.Log.w("PennyWiseApp", "Failed to register SlipMediaObserver: ${e.message}")
        }
